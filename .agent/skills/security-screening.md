...
0. Check local memory approvals first:
   - If token/protocol is in local whitelist (`memory_bank/security/whitelist.json`, optional) => PASS (Trusted) unless explicit block flags exist.
...
8. Apply stablecoin tiering rules (from internal ADR/decisions log, if defined):
